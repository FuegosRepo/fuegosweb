...
interface ViandeSectionProps {
  title: string
  viandes: Product[]
  type: 'classiques' | 'premium'
  icon: LucideIcon
  selectedViandes: string[]
  canSelectMore: boolean
  onToggle: (viandeId: string) => void
}

const ViandeSection = React.memo(function ViandeSection({
  title, viandes, type, icon: Icon, selectedViandes, canSelectMore, onToggle
}: ViandeSectionProps) {
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 400px' }}>
        {viandes.map((viande) => {
          ...
          return (
            ...
            >
              <Card
                ...
                onClick={() => canSelect && onToggle(viande.id)}
              >
...
export function StepViandes() {
  ...
  const handleViandeToggle = useCallback((viandeId: string) => {
    const isSelected = selectedViandes.includes(viandeId)

    if (isSelected) {
      updateViandes(selectedViandes.filter(id => id !== viandeId))
    } else if (selectedViandes.length < maxSelections) {
      updateViandes([...selectedViandes, viandeId])
    }
  }, [selectedViandes, updateViandes, maxSelections])
  ...
  return (
    ...
    >
      ...
      {/* Premium Section */}
      <ViandeSection
        title="Morceaux Premium"
        viandes={premiumProducts}
        type="premium"
        icon={Crown}
        selectedViandes={selectedViandes}
        canSelectMore={canSelectMore}
        onToggle={handleViandeToggle}
      />

      {/* Classiques Section */}
      <ViandeSection
        title="Morceaux Classiques"
        viandes={classiqueProducts}
        type="classiques"
        icon={Star}
        selectedViandes={selectedViandes}
        canSelectMore={canSelectMore}
